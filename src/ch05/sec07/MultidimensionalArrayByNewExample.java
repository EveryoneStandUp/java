...
public class MultidimensionalArrayByNewExample {
	public static void main(String[] args) {
		...
		for(int i=0; i<mathScores.length; i++) {	// 반의 수만큼 반복
			...
			totalMathSum += mathScores[i].length;
			for (int k=0; k<mathScores[i].length; k++) {
				
				totalMathSum += mathScores[i][k];
			}
		}
		double totalMathArg = (double) totalMathSum / totalStudent;
		System.out.println("전체 학생의 수학 평균 점수: " + totalMathArg);
		System.out.println();
		...
		for(int i=0; i<englishScores.length; i++) {
			for(int k=0; k<englishScores[i].length; k++) {
				...
				System.out.println("englisScores[" + i + "][" + k + "]: " + englishScores[i][k]);
			}
		}
		System.out.println();
		// 배열 항목 값 변경
